...
export default function CRMPortal() {
  ...
  const handleResetQuoteStatus = async (quotation: Quotation) => {
    try {
      await supabase.from('quotations').update({ status: 'sent' }).eq('id', quotation.id);
      alert(isAr ? 'تم إعادة تعيين حالة عرض السعر إلى "معلق/مرسل"!' : 'Quotation status reset to Pending/Sent!');
      fetchAll();
    } catch (err: any) {
      alert(err.message || 'Error resetting quote status');
    }
  };
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {q.status === 'pending' || q.status === 'sent' || q.status === 'draft' ? (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => handleResetQuoteStatus(q)}
                          className="bg-gray-100 hover:bg-gray-200 text-gray-700 text-[10px] font-black uppercase px-2.5 py-1.5 rounded-lg transition-all"
                          title="Reset status back to pending/sent"
                        >
                          ↩ Re-open / Reset
                        </button>
                      </div>
